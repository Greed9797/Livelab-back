...
//
// ─────────────────────────────────────────────────────────────────────────────
// CONTRATO DE ASSINATURA — repassar isto pro pessoal do Make
// ─────────────────────────────────────────────────────────────────────────────
//
...
//     body = '{"nome":"Fulano"}'          // exatamente os bytes enviados
...
//   2. Make passa a enviar v2 → o warn de depreciação some do log.
...
export async function webhookMakeCrmRoutes(app) {
  ...
  app.post('/v1/webhooks/make-crm', {
    ...
    if (parsed.version === 'v1') {
      app.log.warn(
        '[make-crm webhook] DEPRECATED: assinatura v1 (só body) — request é replayável. '
        + 'Emissor deve migrar para X-Livelab-Timestamp + X-Livelab-Nonce (ver contrato no topo de webhook_make_crm.js).',
      )
    }
