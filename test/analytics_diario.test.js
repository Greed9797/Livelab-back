...
describe('analytics diario', () => {
  it('propagates pending declarations to dashboard KPIs and temporal series', async () => {
    ...
    expect(res.json().kpis).toMatchObject({ gmv_total: 0, total_lives: 0, horas_live: 0, pedidos_total: 0, pendente_aprovacao: true, gmv_pendente_aprovacao: 19.99, viewers_total: 0, impressoes_pendentes_aprovacao: 500 })
    expect(res.json().gmv_mensal).toEqual([])
    expect(res.json().gmv_diario).toEqual([])
    await app.close()
  })
  it('includes declared reach and views in the funnel, but excludes collisions from combined totals', async () => {
    ...
    expect(res.json().resumo).toMatchObject({ gmv: 0, total_lives: 0, visualizacoes: 0, pedidos: 0 })
    expect(res.json().etapas[0].valor).toBe(0)
    await app.close()
  })
  ...
  it('includes pending submissions in operational totals without commission', async () => {
    ...
    expect(res.statusCode).toBe(200)
    expect(res.json().rows[0]).toMatchObject({ gmv_lives: 100, total_lives: 1, pedidos: 1, horas_live: 1, comissao_apresentadora: 5, gmv_pendente_aprovacao: 200, total_lives_pendentes_aprovacao: 1, pendente_aprovacao: true, impressoes_pendentes_aprovacao: 500, visualizacoes_pendentes_aprovacao: 100 })
    await app.close()
  })
})
