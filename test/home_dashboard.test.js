...
describe('home dashboard', () => {
  it('includes safe pending GMV in operational KPIs without changing franchise revenue', async () => {
    ...
    expect(body.gmv_total_mes).toBe(1600.5)
    expect(body.gmv_lives_mes).toBe(1200.5)
    expect(body.pedidos_total).toBe(6)
    expect(body.horas_live_mes).toBe(10)
    expect(body.lives_mes).toBe(2)
    expect(body.gmv_pendente_aprovacao).toBe(19.99)
    expect(body.total_lives_pendentes_aprovacao).toBe(1)
    expect(body.ranking_apresentadoras_mes[0].gmv_total).toBe(800)
