...
import { liveSaleCommissionLateralSql } from '../src/lib/sale-gmv-sql.js'
...
describe('GET /v1/lives e /v1/lives/:id — comissão da apresentadora vem do motor', () => {
  for (const rota of ["app.get('/v1/lives',", "app.get('/v1/lives/:id',"]) {
    it(`${rota} soma vendas_atribuidas da live e só cai no snapshot como fallback`, () => {
      ...
      expect(bloco).toContain("liveSaleCommissionLateralSql('l')")
      ...
      const sql = liveSaleCommissionLateralSql('l')
      expect(sql).toMatch(/SUM\(va_c\.comissao_apresentadora\)/)
      expect(sql).toMatch(/va_c\.origem = 'live'/)
      expect(sql).toMatch(/va_c\.origem_id = l\.id/)
      expect(sql).toMatch(/NULLIF\(SUM\(va_c\.gmv\), 0\)/)
      expect(sql).toMatch(/COALESCE\(l\.ads_gmv, l\.manual_gmv, l\.fat_gerado\)/)
    })
  }
})
