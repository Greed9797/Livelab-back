...
describe('GET /v1/public/ranking', () => {
  it('returns only safe public fields without authentication', async () => {
    ...
    expect(queryMock.mock.calls[0][0]).toContain("NULLIF(TRIM(t.ranking_publico_nome), '') IS NOT NULL")
    expect(queryMock.mock.calls[0][0]).toContain("!~* '(teste|test|dev|homolog|staging|qa[_ -]?release|release[_ -]?react|teseste|t[e3][^[:alpha:]]*st[e3])'")
