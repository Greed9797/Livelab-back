...
describe('portal own performance DTO', () => {
  it('counts zero-GMV lives, preserves precision and strips financial/private columns', async () => {
    ...
    expect(result.desempenho).toEqual({ total_lives: 2, gmv_lives: 500.25, horas_live: 3.5, gmv_por_hora: 142.93, pedidos: 3, gmv_pendente_aprovacao: 0, total_lives_pendentes_aprovacao: 0 })
  ...
  it('does not manufacture a rate without attributed hours', async () => {
    ...
    expect(result.desempenho).toEqual({ total_lives: 0, gmv_lives: 0, horas_live: 0, gmv_por_hora: null, pedidos: 0, gmv_pendente_aprovacao: 0, total_lives_pendentes_aprovacao: 0 })
  })
})
